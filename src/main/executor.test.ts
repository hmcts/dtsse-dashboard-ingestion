...
describe('executor', () => {
  ...
  test('executes query files and stores results', async () => {
    ...
    jest.doMock(__dirname + '/query/test-query.ts', () => ({
  ...
  test('skips storing when query returns empty results', async () => {
    jest.doMock(__dirname + '/query/empty-query.ts', () => ({
  ...
  test('continues execution when query fails', async () => {
    ...
    jest.doMock(__dirname + '/query/failing-query.ts', () => ({
      run: jest.fn().mockImplementation(() => Promise.reject(new Error('Query failed'))),
    }), { virtual: true });

    jest.doMock(__dirname + '/query/success-query.ts', () => ({
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining('Query failed (failing-query.ts)'),
      expect.any(Error)
    );
  ...
  test('runs migrate before executing queries', async () => {
    jest.doMock(__dirname + '/query/test.ts', () => ({
      run: jest.fn().mockImplementation(() => Promise.resolve([])),
    }), { virtual: true });
  ...
  test('always shuts down connection pool', async () => {
    jest.doMock(__dirname + '/query/error-query.ts', () => ({
  ...
  test('executes multiple queries in parallel', async () => {
    ...
    jest.doMock(__dirname + '/query/query1.ts', () => ({
      run: mockQuery1,
    }), { virtual: true });

    jest.doMock(__dirname + '/query/query2.ts', () => ({
      run: mockQuery2,
    }), { virtual: true });
  ...
  test('logs query execution time', async () => {
    ...
    jest.doMock(__dirname + '/query/timed-query.ts', () => ({
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringMatching(/Finished query: timed-query\.ts in \d+ms/)
    );
