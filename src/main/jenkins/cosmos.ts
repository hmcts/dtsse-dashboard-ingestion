...
export const getCVEs = async (fromUnixtime: bigint, codebaseType: string) => {
  ...
  for (const connection of jenkinsConnections) {
    const querySpec = {
      query: `SELECT * from c where c._ts >= ${fromUnixtime} and c.build.branch_name = "master" and c.build.codebase_type = "${codebaseType}" order by c._ts asc offset 0 limit 2000`,
    };
